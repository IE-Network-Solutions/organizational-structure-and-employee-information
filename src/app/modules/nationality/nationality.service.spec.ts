...
describe('NationalityService', () => {
  ...
  describe('create', () => {
    describe('when create is called', () => {
      ...
      beforeEach(() => {
        nationalityRepository.create.mockReturnValue(
          createNationality() as any,
        );
      ...
      it('should call nationalityRepository.create', async () => {
        ...
        expect(nationalityRepository.create).toHaveBeenCalledWith({
          ...createNationality(),
          tenantId: 'tenant-id',
        });
      });
      ...
      it('should call nationalityRepository.save', async () => {
        await nationalityService.create(createNationality(), 'tenant-id');
        expect(nationalityRepository.save).toHaveBeenCalledWith(
          createNationality(),
        );
      });

      it('should return the created nationality', async () => {
        nationality = await nationalityService.create(
          createNationality(),
          'tenant-id',
        );
  ...
  describe('findAll', () => {
    it('should apply search filters correctly', async () => {
      ...
      jest
        .spyOn(nationalityRepository, 'createQueryBuilder')
        .mockReturnValue(queryBuilderMock as any);
      jest
        .spyOn(paginationService, 'paginate')
        .mockImplementation(paginateMock);

      await nationalityService.findAll(paginationDto);

      expect(nationalityRepository.createQueryBuilder).toHaveBeenCalledWith(
        'nationality',
      );
  ...
  describe('findOne', () => {
    describe('when findOne is called', () => {
      ...
      it('should call nationalityRepository.createQueryBuilder with correct params', () => {
        ...
        expect(
          nationalityRepository.createQueryBuilder().where,
        ).toHaveBeenCalledWith('nationality.id = :id', {
  ...
  describe('update', () => {
    describe('when update is called', () => {
      ...
      beforeEach(async () => {
        nationalityRepository.findOneOrFail.mockResolvedValue(
          nationalityData() as any,
        );
        nationality = await nationalityService.update(
          nationalityData().id,
          updateNationalityData(),
        );
      });
  ...
  describe('remove', () => {
    describe('when remove is called', () => {
      ...
      it('should call nationalityRepository.findOneOrFail with correct params', async () => {
        ...
        expect(nationalityRepository.findOneOrFail).toHaveBeenCalledWith({
          where: { id: nationalityData().id },
        });
      });

      it('should call nationalityRepository.softDelete with correct params', async () => {
        await nationalityService.remove(nationalityData().id);
        expect(nationalityRepository.softDelete).toHaveBeenCalledWith({
          id: nationalityData().id,
        });
      });
    });
