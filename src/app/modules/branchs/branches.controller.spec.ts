...
import { branchData, createbranchData, createbranchDataOnCreate, deletebranchData, findOneNotFoundReturnValue, paginationResultbranchData, updatebranchData } from './tests/branch.data';
...
describe('BranchesController', () => {
  ...
  const mockBranchesService = {
    createBranch: jest.fn().mockImplementation((dto: CreateBranchDto, tenantId: string) => {
      return Promise.resolve(createbranchDataOnCreate());
    }),
    findAllBranchs: jest.fn().mockImplementation((paginationOptions: PaginationDto, tenantId: string) => {
    ...
    updateBranch: jest.fn().mockImplementation((id: string, dto: UpdateBranchDto) => {
  ...
  it('should create a branch', async () => {
    ...
    expect(service.createBranch).toHaveBeenCalledWith(createBranchDto, '8f2e3691-423f-4f21-b676-ba3a932b7c7c');
  });
  ...
  it('should find all branches', async () => {
    ...
    expect(service.findAllBranchs).toHaveBeenCalledWith(paginationOptions, '8f2e3691-423f-4f21-b676-ba3a932b7c7c');
  });

  it('should find one branch', async () => {
    const result = await controller.findOneBranch('be21f28b-4651-4d6f-8f08-d8128da64ee5');
    expect(result).toEqual(branchData());
    expect(service.findOneBranch).toHaveBeenCalledWith('be21f28b-4651-4d6f-8f08-d8128da64ee5');
  });
  ...
  it('should update a branch', async () => {
    ...
    const result = await controller.updateBranch('be21f28b-4651-4d6f-8f08-d8128da64ee5', updateBranchDto);
    expect(result).toEqual(updatebranchData());
    expect(service.updateBranch).toHaveBeenCalledWith('be21f28b-4651-4d6f-8f08-d8128da64ee5', updateBranchDto);
  });

  it('should remove a branch', async () => {
    const result = await controller.removeBranch('be21f28b-4651-4d6f-8f08-d8128da64ee5');
    expect(result).toEqual(deletebranchData());
    expect(service.removeBranch).toHaveBeenCalledWith('be21f28b-4651-4d6f-8f08-d8128da64ee5');
  });
});
