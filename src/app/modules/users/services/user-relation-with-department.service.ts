...
export class UserDepartmentService {
  ...
  async dissolveDepartment(dissolveDepartmentDto: DissolveDepartmentDto,tenantId:string): Promise<Department> {
    try {
      const departmentToDelete= dissolveDepartmentDto.departmentToDelete
      delete dissolveDepartmentDto.departmentToDelete
      const departments =
        await this.departmentService.updateDepartment(dissolveDepartmentDto.id,dissolveDepartmentDto,tenantId);
      if (departments) {
        for(const department of departmentToDelete){
        const departmentUsers= await this.userRepository.find({where:{employeeJobInformation:{departmentId:department,tenantId:tenantId}},relations:['employeeJobInformation']})
        if(departmentUsers){
          for(const user of departmentUsers){
            ...
            await this.employeeJobInformationService.update(departmentUser.id,updatedData)
          }
        }
      }
        }
