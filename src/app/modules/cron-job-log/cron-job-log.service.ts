...
export class CronJobLogService { 
  ...
  constructor( 
    ...
    try {
      ...
      const activeCalendars = await this.calendarsService.findActiveCalendarForAllTenants();
        ...
        if (today >= startDate && today <= endDate) {
          for(const session of activeCalendar.sessions){
            ...
            if (today >= sessionStartDate && today <= sessionEndDate) {
              if(session.active){
                console.log(`Today is within the active calendar period for tenant ${activeCalendar.tenantId}`);
